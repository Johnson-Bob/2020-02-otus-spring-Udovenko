...
class ModelDtoConverter {
  ...
  public static Set<AuthorModel> toAuthorModelSet(Collection<AuthorDto> dtoSet) {
    return dtoSet.stream().map(ModelDtoConverter::toAuthorModel).collect(Collectors.toSet());
  }
  ...
  public static AuthorDto toAuthorDto(AuthorModel model) {
    ...
    return AuthorDto.builder().id(model.getId()).firstName(model.getFirstName())
      .lastName(model.getLastName()).build();
  }
  ...
  public static List<CommentModel> toCommentModelList(Collection<CommentDto> dtoCollection) {
    return dtoCollection.stream().map(ModelDtoConverter::toCommentModel)
  ...
  public static CommentDto toCommentDto(CommentModel model) {
    ...
    return CommentDto.builder().text(model.getText())
      .bookDto(BookDto.builder().id(model.getBookId()).build()).create(model.getCreate()).build();
  }
  ...
  public static BookDto toBookDto(BookModel model) {
    ...
    Set<AuthorDto> authorDtoSet = model.getAuthors().stream().map(ModelDtoConverter::toAuthorDto)
      .collect(Collectors.toSet());

    return BookDto.builder().id(model.getId()).bookTitle(model.getBookTitle())
      .genre(model.getGenre()).authors(authorDtoSet).build();
  }
  ...
  public static List<BookModel> toBookModelList(Collection<BookDto> dtoList) {
    return dtoList.stream().map(ModelDtoConverter::toBookModel).collect(Collectors.toList());
  }
}
