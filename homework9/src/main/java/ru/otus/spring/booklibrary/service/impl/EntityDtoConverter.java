...
class EntityDtoConverter {
  public static BookDto toBookDto(Book book) {
    return BookDto.builder().id(book.getId()).bookTitle(book.getTitle()).genre(book.getGenre())
      .authors(toSetAuthorDto(book.getAuthors())).build();
  }

  public static List<BookDto> toListBookDto(Collection<Book> books) {
    return StreamSupport.stream(books.spliterator(), false).map(EntityDtoConverter::toBookDto)
      .collect(toList());
  }

  public static AuthorDto toAuthorDto(Author author) {
    return AuthorDto.builder().id(author.getId()).firstName(author.getFirstName())
      .lastName(author.getLastName()).build();
  }

  public static Set<AuthorDto> toSetAuthorDto(Collection<Author> authors) {
    return StreamSupport.stream(authors.spliterator(), false).map(EntityDtoConverter::toAuthorDto)
  ...
  public static Set<Author> toAuthorSet(Collection<AuthorDto> dtoSet) {
    return dtoSet.stream().map(EntityDtoConverter::toAuthor).collect(Collectors.toSet());
  }

  public static CommentDto toCommentDto(Comment comment, Book book) {
    return CommentDto.builder().bookDto(toBookDto(book)).text(comment.getText())
      .create(comment.getCreate()).build();
  }
